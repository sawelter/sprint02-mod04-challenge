...
class Car {
  ...
  drive(distance) {
    // sets distance traveled to as far as it can go with the amount of fuel that is in the tank.
    if (distance > (this.tank * this.milesPerGallon)) {
      distance = this.tank * this.milesPerGallon;
    }
    // increases odometer, decreases fuel
    this.odometer += distance;
    this.tank -= distance / this.milesPerGallon;
    // if the car ran out of fuel, it returns a string saying where the odometer is at when it stopped
    if(this.tank <= 0) {
...
class Student extends Lambdasian {
   ...
   listSubjects() {
    ...
    str += this.favSubjects[this.favSubjects.length - 1] + "!"
    return str;
   }
